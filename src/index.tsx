...
// 사이트 운영자가 매일 출근해서 빠르게 훑는 점검 항목 (사용자 지정 8개).
// 항목이 0개일 때 자동 시드되고, 이전 버전의 기본 항목들은 첫 실행 시 정리된다.
const DEFAULT_CHECKLIST_ITEMS: Array<{ name: string; description: string; check_type: 'auto' | 'manual'; endpoint: string; sort_order: number }> = [
  { name: '회원가입 정상 작동 여부',     description: '회원가입 페이지에서 신규 계정 가입을 시도해보고 이메일 인증까지 정상 처리되는지 확인',     check_type: 'manual', endpoint: '',                    sort_order: 10 },
  { name: '로그아웃 정상 작동 여부',     description: '로그아웃 버튼을 눌러 세션이 정상적으로 종료되고 로그인 페이지로 이동하는지 확인',         check_type: 'manual', endpoint: '',                    sort_order: 20 },
  { name: '권한 없는 페이지 접근 차단',  description: '비로그인 상태에서 보호된 페이지에 직접 URL 접근 시 로그인 페이지로 리디렉션되는지 확인',  check_type: 'manual', endpoint: '',                    sort_order: 30 },
  { name: '메인 페이지 로딩',           description: 'winai.kr 루트 페이지 (/) 가 200 응답하는지 자동 확인',                                     check_type: 'auto',   endpoint: '/',                   sort_order: 40 },
  { name: '정적 자원 (CSS) 로딩',       description: '정적 파일 서빙 (/static/styles.css) 이 정상 동작하는지 자동 확인',                         check_type: 'auto',   endpoint: '/static/styles.css',  sort_order: 50 },
  { name: '병원 목록 API',              description: 'GET /api/hospitals — 병원 데이터 조회 정상 응답 확인',                                     check_type: 'auto',   endpoint: '/api/hospitals',      sort_order: 60 },
  { name: 'AI 잔액 API',                description: 'GET /api/ai-balance — Claude / GPT 누적 잔액 API 정상 응답 확인',                          check_type: 'auto',   endpoint: '/api/ai-balance',     sort_order: 70 },
  { name: '병원 관리 탭 정상 표시',     description: '병원 관리 탭에 진입해서 병원 목록이 정상적으로 보이고 빈 화면 / 에러가 없는지 확인',        check_type: 'manual', endpoint: '',                    sort_order: 80 },
]

// 이전 버전 (PR #9 이전) 의 기본 시드 항목 이름 — 첫 실행 시 일괄 정리한다.
// 사용자가 새로 만든 항목 (이 목록에 없는 이름) 은 보존된다.
const LEGACY_DEFAULT_NAMES = new Set<string>([
  '로그인 정상 작동 여부', '비밀번호 찾기 / 재설정', '잘못된 비밀번호 에러 메시지',
  '이번달 일정 API', '이번달 업무 API', '이번달 예산 API', '이번달 AI 사용량 API', '연차/휴가 API',
  '병원 추가/수정/삭제 동작', '캘린더 렌더링 정상', '캘린더에서 일정 클릭 동작',
  '일정 추가 동작', '일정 삭제 동작', '일정 메모 입력/저장',
  '작업 상세 모달 열기/닫기', '하위작업 추가/완료 토글', '작업 진척률 변경 + 기록 저장',
  '간트차트 렌더링', '간트차트 드래그/리사이즈', '연차/휴가 추가/삭제',
  '예산 항목 추가 (수시결제)', '예산 항목 추가 (정기결제)',
  '예산 결제 승인 토글', '예산 결제 완료 토글', '예산 카드별 클릭 패널 이동',
  'AI 사용량 표시 정상', 'AI 잔액 표시 정상', 'AI 사용량 자동 동기화 동작',
  '어제 마감 작업 완료 처리', '결제 승인 대기 누적 점검', 'AI 잔액 부족 임박 여부',
  '전체 레이아웃 / 오타 점검', '모바일 화면 점검', '브라우저 콘솔 에러 확인',
  // 그 이전 버전 잔재
  '병원 목록 조회', '이번달 일정 조회', '이번달 업무 조회', '예산 데이터 조회',
  'AI 사용량 조회', '연차/휴가 조회',
  '캘린더 렌더링', '일정 추가/삭제 동작', '업무 진척률 업데이트',
  '전체 UI 점검',
  '정적 자원 (styles.css)', '결제 승인 대기 항목 확인',
  'AI 잔액 임계치 점검', 'AI 사용량 자동 동기화', '오늘 마감 일정 점검',
  '마감 임박 (3일 이내) 일정 확인', '어제 작업 마감 처리', '진척률 낮은 작업 점검',
  '이번주 휴가/연차 확인', '병원별 마감 임박 검토', '정기결제 예상 청구 점검',
  '모바일 화면 UI 점검', 'Cloudflare 배포 상태 점검',
])
...
async function ensureChecklistTables(db: any) {
  ...
  `).run()
  // 메타 테이블 — 시드 버전을 추적해 코드가 바뀔 때마다 한 번씩만 정리 작업이 돈다.
  await db.prepare(`
  ...
  const CURRENT_SEED_VERSION = '2026-05-15-user-trim'
  const verRow: any = await db.prepare("SELECT value FROM app_meta WHERE key = 'checklist_seed_version'").first()
  const prevVersion = String(verRow?.value || '')

  if (prevVersion !== CURRENT_SEED_VERSION) {
    // 1) 이전 버전의 기본 시드 항목들을 일괄 정리한다 (사용자가 새로 만든 항목은 보존).
    //    함께 해당 항목의 로그도 cascade 삭제된다.
    for (const name of LEGACY_DEFAULT_NAMES) {
      await db.prepare('DELETE FROM checklist_items WHERE name = ?').bind(name).run()
    }
    // 2) 현재 기본 항목 중 없는 것만 새로 추가 (이름으로 중복 확인).
    for (const item of DEFAULT_CHECKLIST_ITEMS) {
      const existing: any = await db.prepare('SELECT id FROM checklist_items WHERE name = ?').bind(item.name).first()
      if (!existing) {
        await db.prepare(
    ...
    // 3) 시드 버전 마커 기록 — 이후 호출에서는 정리 / 시드 로직이 다시 돌지 않는다.
    await db.prepare(`
      INSERT INTO app_meta (key, value) VALUES ('checklist_seed_version', ?)
      ON CONFLICT(key) DO UPDATE SET value = excluded.value
    `).bind(CURRENT_SEED_VERSION).run()
  }
}
